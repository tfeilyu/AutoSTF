...
from src.train_util import Scaler
...
class TFdataProcessing:
    def __init__(self, dataset, train_prop, valid_prop,
        ...
        logging.info('initialize %s DataWrapper', dataset)
    ...
    def read_adj_mat(self):
        ...
        if self.adj_type == "scalap":
            adj = [train_util.calculate_scaled_laplacian(adj_mx)]
        elif self.adj_type == "normlap":
            adj = [train_util.calculate_normalized_laplacian(adj_mx).astype(np.float32).todense()]
        elif self.adj_type == "symnadj":
            adj = [train_util.sym_adj(adj_mx)]
        elif self.adj_type == "transition":
            adj = [train_util.asym_adj(adj_mx)]
