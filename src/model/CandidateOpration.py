...
import numpy as np
import math
from src.model.transformer import LightformerLayer, Lightformer, LearnedPositionalEncoding
...
def create_op(op_name, node_embedding_1, node_embedding_2, adj_mx, config, device):
    name2op = {
        ...
        'MatrixDCRNN': lambda: MatrixDCRNN(config, adj_mx, device),
        'MatrixAdaptive': lambda: MatrixAdaptive(config, node_embedding_1, node_embedding_2),
        'LightGFormer': lambda: LightGFormer(config),
    }
    op = name2op[op_name]()
    return op
...
class MatrixDCRNN(nn.Module):
    ...
    def __init__(self, config, adj_mx, device, use_bn=True, dropout=0.15):
        super(MatrixDCRNN, self).__init__()
        self.use_bn = use_bn
